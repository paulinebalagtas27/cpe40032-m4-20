...
function PlayState:enter(params)
    ...
    -- CRUSH 3 UPDATE: TO PASS THE CURRENT LEVEL TO BOARD FOR THE TILE LEVEL UP UPDATE 
    ...
    self.scoreGoal = self.level * 1.25 * 3000
end
...
function PlayState:update(dt)
    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') 
            ...
            -- CRUSH 3 UPDATE: MOUSE UPDATE, WE ALSO ADDED SPACE FOR EASIER GAMEPLAY IF THE PLAYER WILL ONLY USE KEYBOARD TO PLAY
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        return true
    -- new tiles set if no more match tiles
    elseif not self.board:matchesAvailable() then
            self.printResetMessage = true
            gSounds['no-match']:play()
...
function PlayState:render()
    ...
    -- CRUSH 3 UPDATE: IF NO POSSIBLE MATCHES, THIS WILL APPEAR
    if self.printResetMessage then
        love.graphics.setColor(85, 100, 56, 234)
        love.graphics.rectangle('fill', 16, 140, 185, 30, 4)
        love.graphics.setColor(200, 155, 255, 255)
        love.graphics.printf('No matches available!', 17, 145, 182, 'center')
    end
end